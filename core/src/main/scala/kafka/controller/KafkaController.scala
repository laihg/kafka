...
class KafkaController(val config: KafkaConfig,
  ...
  //事件管理器，Controller中所有的事件都放到事件管理器中的队列中，由单个线程从队列中获取事件进行处理。
  // visible for testing
  ...
  /**
   * 当前broker被选举为controller，需要做以下事情：
   * 1.初始化controller上下文对象，该对象包含所有现有分区的当前主题、存活的broker和leader副本的缓存对象。
   * 2.启动controller channel 管理对象
   * 3.启动副本
   * 4.启动分区
   * 5.如果当前broker在成为Controller时发生了异常错误，当前controller会被辞退掉，确保可以触发其他的broker进行下一轮的controller选举。
  ...
  private def initializeControllerContext(): Unit = {
    ...
    //注册topic partition修改变更事件
    registerPartitionModificationsHandlers(controllerContext.allTopics.toSeq)

    //根据topic获取topic partition分配信息
    ...
    //从 Zookeeper 更新所有现有分区的领导者和 isr 缓存
